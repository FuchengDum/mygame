...
export default memo(function ActiveBuffIndicator({ buffs }: Props) {
  ...
  return (
    <div
      className="absolute left-4 top-[11rem] sm:top-44 flex flex-col gap-1.5 z-10"
      role="region"
      aria-label="激活的增益效果"
    >
      {buffs.map(buff => {
        ...
        const percent = buff.totalMs > 0
          ? Math.max(0, Math.min(100, (buff.remainingMs / buff.totalMs) * 100))
          : 0
        ...
        return (
          ...
          >
            <span className="text-base w-5 text-center" role="img" aria-label={config.label}>{config.icon}</span>
            <div
              className="w-14 h-1.5 bg-gray-700 rounded-full overflow-hidden"
              role="progressbar"
              aria-valuenow={Math.round(percent)}
              aria-valuemin={0}
              aria-valuemax={100}
              aria-label={`${config.label}剩余时间`}
            >
              <div
