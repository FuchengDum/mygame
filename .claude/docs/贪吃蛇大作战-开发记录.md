...
**需求**：特殊食物仅用颜色区分，对色盲用户不友好，需要增加图标辨识度

**实现方案**：
- 运行时生成纹理，为每种特殊食物设计独特图标
- 小地图使用不同形状标记不同类型特殊食物
...
| 类型 | 图标 | 颜色 | 小地图形状 |
|------|------|------|------------|
| speed | ⚡ 闪电 | 青色 `0x00f5ff` | 三角形 △ |
| slow | ⏳ 沙漏 | 紫色 `0xbf00ff` | 菱形 ◇ |
| double | 2x 文字 | 粉色 `0xff00aa` | 方形 □ |
| magnet | 🧲 U形磁铁 | 蓝色 `0x00aaff` | 圆形 ○ |
| poison | ☠ 骷髅 | 红色 `0xff0044` | X形 ✕ |
...
private applySelfCollisionPenalty() {
  ...
  this.collisionPenaltyUntilMs = now + 3000  // 3秒冷却
  this.speedMultiplier = 0.5                  // 减速惩罚
  this.speedBuffUntilMs = now + 3000
  this.shrinkSnake(4)                         // 缩短蛇身
  this.score = Math.max(0, this.score - 30)   // 扣分

  // 视觉反馈：蛇头闪红
  const head = this.snake[0]
  if (head) {
    head.graphics.setFillStyle(0xff0044)
    this.time.delayedCall(100, () => head.graphics.setFillStyle(this.HEAD_COLOR))
    this.time.delayedCall(200, () => head.graphics.setFillStyle(0xff0044))
    this.time.delayedCall(300, () => head.graphics.setFillStyle(this.HEAD_COLOR))
  }
}
```
...
**问题描述**：游戏平台首页横屏时，需要往下滑动才能看到游戏名称

**根因分析**：
- 横屏时屏幕高度有限（约 375px）
- 搜索栏和分类筛选占用过多垂直空间
- 游戏卡片 `aspect-[4/3]` 比例过高
- 固定 2 列布局未充分利用横向空间
...
**问题来源**：多模型代码审查（Codex + Gemini）

**修复内容**：

| 问题 | 修复方案 |
|------|----------|
| fetch 无错误处理 | 添加 `res.ok` 检查和 `.catch()`，显示错误提示 |
| GameCard 键盘不可访问 | 改用 `<button>` 元素，添加 `aria-label` 和 focus 样式 |
| 无效的 `_double_text_template` | 删除未使用的隐藏文本对象 |

**涉及文件**：
- `src/pages/HomePage.tsx` - fetch 错误处理、GameCard 可访问性
- `src/game/games/SnakeGame/SnakeScene.ts` - 删除无效对象
...
// GameCard 改用 button 元素
function GameCard({ game, onClick, isLandscape }) {
  return (
    <button
      onClick={onClick}
      aria-label={`${game.name} - ${game.category}`}
      className="... focus:outline-none focus:ring-2 focus:ring-neon-blue"
    >
      ...
    </button>
  )
